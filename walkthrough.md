...
**Passo 3:** Abra o `.astro` gerado e faça os ajustes manuais obrigatórios:

- Envolva todo o conteúdo com o layout `<Base>` (veja estrutura abaixo)
- Substitua imagens brutas pelo componente `<Img>` (seção 4)
- Substitua o `<form>` bruto pelo componente `<LeadForm>` (seção 6)
- Substitua vídeos pelo componente `<Video>` (seção 5)
